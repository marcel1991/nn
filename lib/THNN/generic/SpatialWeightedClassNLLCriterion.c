...
#define TH_GENERIC_FILE "generic/SpatialWeightedClassNLLCriterion.c"
#else
          ...
          THTensor *total_weight)
{
  ...
  spatialWeights = spatialWeights ? THTensor_(newContiguous)(spatialWeights) : NULL;

  real *input_data = THTensor_(data)(input);
  THIndex_t *target_data = THIndexTensor_(data)(target);
  real *weights_data = weights ? THTensor_(data)(weights) : NULL;
  real *spatialWeights_data = spatialWeights ? THTensor_(data)(spatialWeights) : NULL;
  ...
  for (int b = 0; b < batch_size; b++) {
    for (int elem = 0; elem < map_size; elem++) {
      ...
      real cur_spWeight = spatialWeights ? spatialWeights_data[b * map_size + elem] : 1.0f;
      total_weight_acc += cur_weight * cur_spWeight;
      output_acc -= input_data[b * sample_size + cur_target * map_size + elem] * cur_weight * cur_spWeight;
    }
  }
  ...
  for (b = 0; b < batch_size; b++) {
    ...
    for (elem = 0; elem < map_size; elem++) {
      ...
      gradInput_data[b * sample_size + cur_target * map_size + elem] =
        -((weights ? weights_data[cur_target] : 1.0f)*(spatialWeights ? spatialWeights_data[b * map_size + elem] : 1.0f)) / normalize;
    }
  }
